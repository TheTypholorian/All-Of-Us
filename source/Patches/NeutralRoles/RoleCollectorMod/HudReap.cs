...
namespace TownOfUs.NeutralRoles.RoleCollectorMod
{
    ...
    public class HudReap
    {
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            if (!PlayerControl.LocalPlayer.Is(RoleEnum.RoleCollector)) return;

            var role = Role.GetRole<RoleCollector>(PlayerControl.LocalPlayer);
            ...
            if (role.CollectedText != null)
            {
                role.CollectedText.text = role.RolesCollected + "/" + CustomGameOptions.MaxRoles + "";
            }
            ...
            {
                if (!flag || isDead || collider2D.gameObject.name != "Role") continue;
                var component = collider2D.GetComponent<Role>();
                ...
                closestRole = component;
                closestDistance = distance;
            }

            KillButtonTarget.SetTarget(killButton, closestRole, role);
